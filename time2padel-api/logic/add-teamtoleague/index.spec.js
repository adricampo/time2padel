...
describe('logic - add team to league', () => {
    ...
    it('should fail if number of teams in the league is already 6', async () => { 
        ...
        try{
            ...
            expect(error.message).to.equal(`Sorry, this league is complete`)
        }
    })
    ...
    after(() => Promise.all([Team.deleteMany(), User.deleteMany(), League.deleteMany(), PlayingDay.deleteMany(), Match.deleteMany()]).then(database.disconnect))
